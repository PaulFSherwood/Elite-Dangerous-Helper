...
def asteroid_location_available(
    *,
    ...
    """Return whether an Asteroid Base has a physically valid free slot.

    Elite treats Asteroid Base capacity as a constrained subset of ordinary
    orbital capacity.  A valid location therefore needs a ring/belt environment
    and free capacity in both the Orbital and Asteroid counters.
    """
    if not has_environment:
        return False
    return (
        int(orbital_used or 0) < int(orbital_total or 0)
        and int(asteroid_used or 0) < int(asteroid_total or 0)
    )
